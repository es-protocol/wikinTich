/**
 * Parent Cancel Session API Route
 *
 * Handles cancelling a session by a parent.
 * Uses server-side supabaseAdmin to bypass RLS restrictions.
 *
 * Security:
 * - Session validation required
 * - Role check: only parents can access
 * - Parents can only cancel sessions where they are the parent_id
...
import { isValidUUID, canModifySession } from '@/lib/services/session-validation-service'
...
import { notifyOfCancellation } from '@/lib/services/session-notification-service'
...
async function getTutorName(
  ...
  return profile?.full_name || 'Tutor'
}

// ============================================
// PATCH Handler - Cancel Session
// ============================================
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    // Sanitize reason
    const sanitizedReason = sanitizeTextInput(body.reason).substring(
      0,
      SESSION_VALIDATION.MAX_CANCELLATION_REASON_LENGTH
    )

    devLog('Parent cancelling session:', sessionId)
    ...
    const { data: updatedSession, error: updateError } = await adminClient
      ...
      .update({
        ...
        cancelled_by: SESSION_CREATOR.PARENT,
    ...
    const tutorName = await getTutorName(adminClient, tutoringSession.tutor_id)
    ...
    devLog('Session cancelled by parent:', sessionId)
    ...
    devError('Unexpected error in cancel session:', error)
    return applySecurityHeaders(
